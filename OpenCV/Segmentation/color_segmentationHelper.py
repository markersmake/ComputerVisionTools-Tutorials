...
while True:
    ...
    mask = cv2.morphologyEc(mask, cv2.MORPH_CLOSE, kernel5)
    output = np.zeros(mask.shape, np.uint8)
    rect = cv2.bounding(mask)
    cursor_x = rect[0]
    cursor_y = rect[1]

    cv2.putText(output, "H:" + str(H) + " S:" +str(S)+" V:"+str(V), (10,30), cv2.FONT_HERSHEY_PLAN, 2.0, (255,255,255), thickness = 1)
    cv2.putText(output, "X:" + str(cursor_x)+" Y:"+ str(cursor_y), (10,50), cv2.FONT_HERSHEY_PLAIN, 2.0, (255,255,255), thickness = 1)
    output = draw_points(output, cursur_x, cursor_y, 3,(255,255,255))
